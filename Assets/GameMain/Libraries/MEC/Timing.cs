...
namespace MEC
{
    public class Timing : MonoBehaviour
    {
        ...
        {
            return _handleToIndex.ContainsKey(handle) && !CoindexIsNull(_handleToIndex[handle]) && !SetPause(handle) ? 1 : 0;
        }
        ...
        public int PauseCoroutinesOnInstance(string tag)
        {
            ...
            while (matchesEnum.MoveNext())
                if (!CoindexIsNull(_handleToIndex[matchesEnum.Current]) && !SetPause(matchesEnum.Current))
                    count++;
        ...
        public int ResumeCoroutinesOnInstance()
        {
            ...
            while (waitingEnum.MoveNext())
            {
                ...
                while (pausedList.MoveNext())
                {
                    ...
                    {
                        SetPause(pausedList.Current.Handle);
                        listCount++;
                    }
                    else
                    {
        ...
        public int ResumeCoroutinesOnInstance(CoroutineHandle handle)
        {
            ...
            return _handleToIndex.ContainsKey(handle) &&
                !CoindexIsNull(_handleToIndex[handle]) && SetPause(handle, false) ? 1 : 0;
        }
        ...
        public int ResumeCoroutinesOnInstance(string tag)
        {
            ...
            while (indexesEnum.MoveNext())
                if (!CoindexIsNull(_handleToIndex[indexesEnum.Current]) && SetPause(indexesEnum.Current, false))
                    count++;
        ...
        private bool SetPause(CoroutineHandle handle, bool newPausedState = true)
        {
            return SetPause(_handleToIndex[handle], newPausedState);
        }
        ...
        private IEnumerator<float> WaitUntilDoneWrapper(IEnumerator<float> coptr, CoroutineHandle handle)
        {
            ...
            _waitingTriggers[_tmpHandle].Add(new ProcessData
            {
                Handle = handle,
        ...
        private void CloseWaitingProcess(CoroutineHandle handle)
        {
            ...
            {
                if (_handleToIndex.ContainsKey(tasksEnum.Current.Handle) && !HandleIsInWaitingList(tasksEnum.Current.Handle))
                {
        ...
        private bool HandleIsInWaitingList(CoroutineHandle handle)
        {
            ...
            {
                var otherTasksEnum = triggersEnum.Current.Value.GetEnumerator();
                while (otherTasksEnum.MoveNext())
                    if (otherTasksEnum.Current.Handle == handle)
                        return true;
            }
